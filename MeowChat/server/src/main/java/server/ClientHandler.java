...
public class ClientHandler {
    ...
    private final int TIMEOUT_TO_KICK = 150000;
                    ...
                    while (true) {
                        ...
                        if (msg.startsWith(Command.AUTH)) {
                            ...
                            if (checkNick != null) {
                                if (!server.isLoginIsAuthenticated(login)) {
                                    nickname = checkNick;
                                    //sendMsg(String.format("%s %s %s", Command.AUTH_SUCCESS, nickname, loadUserHistory(login, HISTORY_LIMIT)));
                                    sendMsg(String.format("%s %s", Command.AUTH_SUCCESS, nickname));
                                    server.subscribeClient(this);
                                    System.out.printf(Command.CLIENT_CONNECTED, socket.getRemoteSocketAddress(), nickname);
                                    socket.setSoTimeout(0);
    ...
    private String loadUserHistory(String login, int limit) {
        int user_id = ((DBWorkService) server.getAuthService()).getUserIDByNickname(login);
        StringBuilder sbHistory = new StringBuilder();
        ResultSet allHistory = ((DBWorkService) server.getAuthService()).loadUserHistory(user_id, limit);
        try {
            while (allHistory.next()) {
                sbHistory.append(allHistory.getString("msg"));
            }
